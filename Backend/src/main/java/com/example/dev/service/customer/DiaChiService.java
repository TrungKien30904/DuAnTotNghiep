...
//                .coupon(coupon)
            ...
            return String.valueOf(40000 * listItems.size());
//            return "{\"error\":\"" + e.getMessage() + "\"}";
    ...
    public void addNewAddress(DiaChi diaChi, Integer idHoaDon, Authentication auth) {
        try {
            HoaDon invoiceFind = hoaDonRepository.findById(idHoaDon).orElseThrow();
            String provinceName,districtName,wardName = "";
            if (invoiceFind.getKhachHang() != null) {
                List<DiaChi> listDiaChi = diaChiRepo.findByKhachHang_IdKhachHang(diaChi.getKhachHang().getIdKhachHang());
                ...
                // lay dia chi truyen vao hd
                provinceName = getProvince(diaChi.getThanhPho());
                districtName = getDistricts(diaChi.getThanhPho(), diaChi.getQuanHuyen());
                wardName = getWards(diaChi.getQuanHuyen(), diaChi.getXaPhuong());
                UserLogin user = (UserLogin) auth.getPrincipal();
                invoiceFind.setNhanVien(nhanVienRepo.findById(user.getId()).orElse(null));
                invoiceFind.setTinhThanhPho(diaChi.getThanhPho());
                invoiceFind.setQuanHuyen(diaChi.getQuanHuyen());
                invoiceFind.setXaPhuong(diaChi.getXaPhuong());
                invoiceFind.setDiaChiChiTiet(diaChi.getDiaChiChiTiet() + "," + provinceName + "," + districtName + "," + wardName);
                invoiceFind.setTenNguoiNhan(diaChi.getTenNguoiNhan());
                invoiceFind.setEmail(invoiceFind.getEmail());
                invoiceFind.setSoDienThoai(diaChi.getSoDienThoai());
                invoiceFind.setGhiChu(diaChi.getGhiChu());
                hoaDonRepository.save(invoiceFind);
            }

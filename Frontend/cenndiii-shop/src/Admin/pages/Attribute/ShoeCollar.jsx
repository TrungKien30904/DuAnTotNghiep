...
export default function ShoeCollar() {
  ...
  const handleSaveEdit = async (updatedRow) => {
    const response = await api.post("/admin/co-giay/sua", {
      idCoGiay: updatedRow.idCoGiay,
      ten: updatedRow.ten,
  ...
  const fetchShoeCollars = async () => {
    try {
      const response = await api.get("/admin/co-giay/hien-thi");
      ...
      console.error("Lỗi khi lấy dữ liệu cổ giày:", error);
    }
  };
  ...
  const columns = [
    ...
    { field: 'ten', headerName: 'Tên cổ giày', flex: 1 },
    {
      field: 'trangThai',
  ...
  return (
    <Box sx={{ maxWidth: 1200, margin: '0 auto', padding: 3 }}>
      ...
      <Breadcrumbs
        ...
        <Typography color="text.primary">Cổ giày</Typography>
      </Breadcrumbs>

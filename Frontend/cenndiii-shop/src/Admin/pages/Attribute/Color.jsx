...
export default function Color() {
  ...
  const handleSaveEdit = async (updatedRow) => {
    const response = await api.post("/admin/mau-sac/sua", {
      idMauSac: updatedRow.idMauSac,
      ten: updatedRow.ten,
  ...
  const fetchColors = async () => {
    try {
      const response = await api.get("/admin/mau-sac/hien-thi");
  ...
  const columns = [
    ...
    { field: 'ten', headerName: 'Tên màu sắc', flex: 1 },
    {
      field: 'trangThai',

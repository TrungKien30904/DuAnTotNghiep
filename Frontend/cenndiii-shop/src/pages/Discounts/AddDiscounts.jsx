...
export default function AddDiscounts() {
  ...
  useEffect(() => {
    if (!selectedIds || (selectedIds && selectedIds.length <= 0)) return;
    console.log(2222,selectedIds)
  ...
  const fetchMaDGG = async () => {
    try {
      ...
      addDGG({ ...addMaDGG, idDotGiamGia: maDGG })
    } catch (error) {
  ...
  const fetchSanPham = async (skip, limit) => {
    ...
    setTotalPages(Math.trunc(response.data.total / limit) + 1); // Tính tổng số trang
  };
        ...
        <div className="grid grid-cols-5 gap-4">
          <div className="relative text-sm col-span-2">
            ...
            <input
              ...
              type="text"
              name="maDotGiamGia"
              value={addMaDGG.idDotGiamGia}
              readOnly
              onChange={(e) => addDGG({ ...addMaDGG, idDotGiamGia: e.target.value })}
                  ...
                  <tbody>
                    {sanPhams.map((sanPham, index) => (
                      <tr key={sanPham.idSanPham} className="border-t text-center">
                        ...
                        <td className="p-2">{sanPham.idSanPham}</td>
              ...
              <tbody>
                {sanPhamChiTiets.map((sanPhamChiTiet, index) => (
                  <tr key={sanPhamChiTiet.idChiTietSanPham} className="border-t">
                    ...
                    <td className="p-2">{sanPhamChiTiet.sanPham.idSanPham}</td>

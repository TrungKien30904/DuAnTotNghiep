...
export default function DanhMucManagement() {
  ...
  const fetchDanhMucs = async () => {
    try {
      const response = await axios.get("http://localhost:8080/admin/danh-muc/hien-thi", {
        params: {
          ten: searchName,
      ...
      if (Array.isArray(response.data)) {
        ...
        setDanhMucs([]); // Đặt về mảng rỗng nếu không phải là mảng
      }
    } catch (error) {
  ...
  const handleSaveChanges = async (id) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/admin/danh-muc/sua/${id}`, {
        ten: newDanhMucName,
        trangThai: newDanhMucStatus,
      });
      setDanhMucs(response.data);
      ...
      if (error.response && error.response.data) {
        ...
        setErrorMessage("Sửa danh mục thất bại"); // Thông báo lỗi chung
      }
      Notification("Sửa danh mục thất bại", "error");
    }
  };

  const openEditModal = (danhMuc) => {
    setDanhMucToEdit(danhMuc);
    setNewDanhMucName(danhMuc.ten);
    setNewDanhMucStatus(danhMuc.trangThai);
    setIsEditModalOpen(true);
  };

  const openAddModal = () => {
    setIsAddModalOpen(true);
    setNewItemName(''); // Reset ô input tên danh mục
  };
  ...
  const handleAddItem = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/admin/danh-muc/them', {
        ten: newItemName,
        trangThai: true,
      });
      ...
      fetchDanhMucs(); // Cập nhật danh sách danh mục
    } catch (error) {
      if (error.response && error.response.data) {
        setErrorMessage(error.response.data.ten || "Lỗi không xác định"); // Hiển thị thông báo lỗi
      } else {
        setErrorMessage("Thêm danh mục thất bại"); // Thông báo lỗi chung
      }
    }
  };
  ...
  const handleRefresh = () => {
    ...
    fetchDanhMucs(); // Gọi lại hàm fetchDanhMucs để lấy dữ liệu mới
  };
  ...
  return (
    <div className="p-2 space-y-4 text-sm">
      ...
      <nav className="text-gray-500 mb-4">
        ...
        &gt;{" "}
        <span className="font-semibold text-black">Quản lý Danh Mục</span>
      </nav>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-sm font-semibold">Danh Sách Danh Mục</h2>
        ...
        {currentItems.length === 0 ? ( // Kiểm tra nếu không có dữ liệu
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {currentItems.map((danhMuc, index) => (
                <tr key={danhMuc.idDanhMuc} className="border-t">
                  <td className="p-2">{index + 1 + indexOfFirstItem}</td>
                  <td className="p-2">{danhMuc.ten}</td>
                  <td className="p-2">
                    <span className={`px-2 py-1 rounded text-white ${danhMuc.trangThai ? "bg-green-500" : "bg-red-500"}`}>
                      {danhMuc.trangThai ? "Đang Bán" : "Ngừng Bán"}
                    </span>
                  </td>
                  <td className="p-2 flex space-x-2">
                    <button
                      className="text-black p-1 rounded"
                      onClick={() => openEditModal(danhMuc)}
                    >
      ...
      >
        <div className="bg-white p-6 rounded-lg shadow-lg w-96">
          <div className="mb-4">
            <h2 className="text-xl font-bold mb-1 text-left">Cập nhật thông tin danh mục</h2>
          </div>
          <div className="mb-4">
            <label htmlFor="danh-muc-name" className="block text-sm font-semibold">
              Tên danh mục mới
            </label>
            <input
              id="danh-muc-name"
          ...
          <div className="mb-4">
            <label className="flex items-center text-sm">
              ...
              <input
                type="checkbox"
                checked={newDanhMucStatus}
                onChange={(e) => setNewDanhMucStatus(e.target.checked)}
                className="ml-2"
              />
            </label>
          </div>
          ...
          <div className="flex justify-end space-x-2">
            ...
            <button
              ...
              onClick={() => handleSaveChanges(danhMucToEdit.idDanhMuc)}
            >
              Xác nhận
            </button>
          </div>
      ...
      >
        <div className="bg-white p-6 rounded-lg shadow-lg w-96">
          <h2 className="text-xl font-bold mb-4">Thêm thông tin danh mục</h2>
          <input
            type="text"
            value={newItemName}
